...
silent autocmd BufEnter,BufRead,BufEnter * if filereadable(expand('%:p')) | silent! execute '!'
					\. 'if [ -d $JPROC_DIR ]; then;'
						\. 'echo >$JPROC_VIM_PROG'
						\. 'echo >$JPROC_VIM_ARG'
						\. 'echo %:p >$JPROC_VIM_FILE_LAST'
					\. 'fi &' | endif
...
function ExitCmd(dir, prog, arg)
	silent! execute '!'
			\. 'if [ -d $JPROC_DIR ]; then;'
				\. 'echo '.a:dir.' >$JPROC_LF_DIR;'
				\. 'echo '.a:prog.'>$JPROC_VIM_PROG;'
				\. 'echo '.a:arg.' >$JPROC_VIM_ARG;'
			\. 'fi'
	xit
endfunction
